...
- When asked to create or edit a Django app, keep Django's default batteries-included app structure and conventions intact. Do not replace, rename, flatten, or otherwise alter default Django app files or formats unless explicitly instructed.
- Do not create git commits unless explicitly told to do so.
...
app/                  Core pages, CMS pages, organizers, partners, sponsors, social links
...
events_and_activities/ Event and tag models, event listing/detail views, event template filters
templates/
...
- **MemberIdSequence** (`membership`) — singleton table for race-safe member ID generation
- **Member** (`membership`) — registered members with auto-generated ID (DM-XXX format). Fields: name, email, phone, gender, year_of_birth, experience_level, primary_language, receive_regular_updates, receive_email_communications. ID generated via `MemberIdSequence.get_next_id()` with `select_for_update()` locking.
- **Tag** (`events_and_activities`) — event categorisation (name)
- **Event** (`events_and_activities`) — community events (name, slug, date, rsvp_link, details, tags M2M)
- **Page** (`app`) — CMS pages edited with Summernote WYSIWYG (title, slug, content, updated_at)
- **Organizer** (`app`) — team page organizers with roles, location, photo, social links, and ordering
- **Partner** (`app`) — partner logos/details, optionally associated with events
- **Sponsor** (`app`) — sponsor logos/details, tier, optionally associated with events
- **SocialLink** (`app`) — social media links rendered in footer (name, icon_class, url, order)
...
- **Media files**: `MEDIA_ROOT = BASE_DIR / 'media'` (organizer, partner, sponsor, and Summernote uploads)
- **Admin**: `/dashboard/` — branded with project logo and color scheme
...
## Current Repository Notes (reviewed 2026-05-10)

These notes reflect the current codebase and should be treated as clarifications where older documentation differs.

### Important Drift From Earlier Notes

- The Django admin is mounted at `/dashboard/` in `config/urls.py`, not `/admin/`.
- Event detail pages are available at `/event/<slug>/`; `Event.slug` is unique and auto-generated when blank.
- The main navigation includes `/team/` and `/apps/`.
- The current model set is split across apps: `membership` owns `MemberIdSequence` and `Member`; `events_and_activities` owns `Tag` and `Event`; `app` owns `Page`, `Organizer`, `Partner`, `Sponsor`, and `SocialLink`.
- `Member` now includes `receive_regular_updates` and `receive_email_communications` opt-in fields.
- `requirements.txt` includes `python-telegram-bot` and `httpx`, but no Telegram bot code or eligibility API endpoints are currently present in the checked application files.
- `Dockerfile` currently uses `python:3.12-slim`, while the project documentation describes Python 3.14. Verify runtime expectations before changing Docker or dependency constraints.
...
/membership/join/send-id/<member_id>/       Email member ID
/membership/lookup/                         Member lookup
/membership/lookup/request-details/<id>/    Data access request email flow
/membership/lookup/request-deletion/<id>/   Data deletion request email flow
...
- `Event` lives in `events_and_activities.models` and has `name`, `slug`, `tags`, `date`, `rsvp_link`, and `details`; default ordering is ascending by date.
- `Organizer` powers the `/team/` page and supports photo plus GitHub, LinkedIn, Twitter, and website links.
- `Partner` and `Sponsor` can be associated with events through many-to-many relationships; both support optional logos and ordering.
- `Sponsor` has tier choices: platinum, gold, silver, bronze, and community.
- `MemberIdSequence` lives in `membership.models`; `get_next_id()` starts at `DM-000`; the concurrency test is skipped on SQLite because SQLite does not support the same concurrent write behavior.

### Template And Frontend Patterns

- Templates extend `templates/layout/base.html` and use Bootstrap 5.3.8 plus Bootstrap Icons from CDN.
- Site-wide theme tokens live in `static/css/styles.css`; admin-specific branding lives in `static/css/admin.css`.
- Event list templates use the custom `truncate_sentences` filter from `events_and_activities/templatetags/text_utils.py`.
- `social_links` is injected globally via `app/context_processors.py` and rendered in the footer.

### Testing Notes

- Use `python manage.py test` for the current test suite.
- Existing `membership/tests.py` tests focus on member ID generation, uniqueness, non-reuse after deletion, and database-backed concurrency behavior.
- When changing member ID logic or models, add or update tests in `membership/tests.py` and run migrations checks with `python manage.py makemigrations --check --dry-run` when appropriate.

### Local Secrets And Generated Files

- A local `.env` file is present but should be treated as private local configuration.
- Do not commit local database files, media uploads, collected static output, virtualenv contents, IDE metadata, or `.env` secrets.
- Preserve the repository rule above: do not delete or remove existing file content without explicit permission.
