...
class Event(models.Model):
    ...
    cfp_deadline = models.DateTimeField(
        null=True,
        blank=True,
        help_text='Latest moment speakers can submit proposals. Leave blank for no cutoff.',
    )
    cfv_deadline = models.DateTimeField(
        null=True,
        blank=True,
        help_text='Latest moment volunteers can sign up. Leave blank for no cutoff.',
    )
    ...
    def cfp_deadline_passed(self):
        return self.cfp_deadline is not None and self.cfp_deadline < timezone.now()

    @property
    def cfv_deadline_passed(self):
        return self.cfv_deadline is not None and self.cfv_deadline < timezone.now()
